    ...
    Request performRequest() throws BridgeException {
        try {
            ...
            try {
                ...
                LogCompat.d(Request.this, "%s %s status: %s %s", Method.name(method()),
                ...
                checkCancelled();
                response = new Response(data, url(), responseCode, responseMessage,
                        responseHeaders, builder.didRedirect, builder.redirectCount);
                ...
                LogCompat.d(Request.this, "%s %s request completed successfully.",
                        Method.name(method()), url());
            } catch (Exception fnf) {
                LogCompat.e(Request.this, "Processing exception... %s, %s",
                        fnf.getClass().getName(), fnf.getMessage());
                ...
                try {
                    ...
                    response = new Response(BridgeUtil.readEntireStream(es), url(), responseCode,
                            responseMessage, responseHeaders, builder.didRedirect, builder.redirectCount);
                } catch (Throwable e3) {
                    LogCompat.e(Request.this, "Unable to get error stream... %s", e3.getMessage());
                    response = new Response(null, url(), responseCode, responseMessage,
                            responseHeaders, builder.didRedirect, builder.redirectCount);
                } finally {
